...
void litehtml::el_table::parse_styles(bool is_reparse)
{
	...
	if(m_border_collapse == border_collapse_separate)
	{
		...
		m_css_border_spacing_z.fromString(get_style_property(_t("-litehtml-border-spacing-z"), true, _t("0px")));
		...
		m_border_spacing_z = doc->cvt_units(m_css_border_spacing_z, fntsz);
	} else
	{
		m_border_spacing_x	= 0;
		m_border_spacing_y	= 0;
		...
		m_css_padding.front.set_value(0, css_units_px);
		m_css_padding.back.set_value(0, css_units_px);
	}
}
